...
function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full py-2 z-50 duration-300 ${headerWhite ? 'bg-backgroundPrimary' : '' }`}>
        ...
        <div>
          <Link href="#" className='header-logo-container'>
            ...
            <Image 
              ...
              alt='Renadyl'
              className={`duration-300 header-logo ${headerWhite ? 'opacity-0' : '' }`}
            />

          </Link>
        </div>
        <div>
          <button className={`duration-300 menu-btn ${isMenuOpen ? 'menu-open' : ''} ${headerWhite ? 'header-white' : '' }`} onClick={handleMenuBtn}>
