...
function HomeSlider() {
  ...
  const startSlideInterval = () =>{
    changeSlideInterval.current = setInterval(()=>{
      setCurrentSlide(prevSlide => (prevSlide + 1) % dotElementsRef.current.length);
    },5000);
  };
    ...
    <div className='relative h-full overflow-hidden'>
        <div className='relative h-full flex justify-center content-center items-center'>
            <div className='aspect-video absolute min-h-full w-content hero-slide active-slide'>
              <video muted playsInLine controls={false} disablePictureInPicture={true} disableRemotePlayback={true} className='h-full w-auto aspect-video pointer-events-none'>
