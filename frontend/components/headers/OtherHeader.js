...
function OtherHeader({currentLocale}) {
  ...
  return (
    <header className={`fixed block top-0 left-0 w-full h-[75px]  text-foregroundPrimary z-10 bg-backgroundPrimary ${headerWhite ? 'shadow-lg' : ''} transition-all duration-300  header-white`}>
        <div className='relative flex max-w-[1200px] mx-auto justify-between items-center content-center h-full py-2 max-md:px-5 max-[1200px]:px-4'>
            ...
            <div className='w-3/5 max-md:w-1/2'>
                ...
                <div className='hidden max-md:flex justify-end'>
                    <button title='Burger menu' onClick={handleMenuBtnClick} className={`burger-menu ${isMenuOpen ? 'opened' : ''}`} >
        ...
        <div className={`hidden max-md:block h-screen w-full bg-backgroundPrimary50 text-foregroundPrimary transition-all duration-300 ${isMenuOpen ? 'translate-x-0 pointer-events-auto' : 'translate-x-[100vw] pointer-events-none'}`}>
