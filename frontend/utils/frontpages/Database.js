...
class Database {
    ...
    async update(table, values, where) {
        const sets = Object.keys(values).map((key, index) => `${key} = $${index + 1}`).join(', ');
        const params = [...Object.values(values), ...Object.values(where)];
        const whereClause = Object.keys(where).map((key, index) => `${key} = $${index + Object.keys(values).length + 1}`).join(' AND ');
